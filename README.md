...
원신 **올인원 준종결표 (ver 7.0)** 스프레드시트를 캐릭터별로 보기 좋게 정리한 반응형 웹 뷰어입니다.
...
사이트 상단 버전 표기(`ver 7.0`)는 `sheet.csv` 첫 줄에서 자동으로 읽어오므로 따로 고칠 필요가 없습니다.

> **CSV가 깨져 보인다면?** 구글 시트 export는 UTF-8(BOM 없음)이라 엑셀·메모장이 CP949로 잘못 읽는 것뿐이고, 파일 자체는 정상입니다. 빌드 스크립트는 UTF-8로 읽으므로 그대로 커밋하면 됩니다. 굳이 엑셀에서 보려면 *데이터 → 텍스트/CSV 가져오기*에서 인코딩을 **UTF-8**로 지정하세요.
...
- 데이터: **원신 올인원 준종결표 ver. 7.0** — 제작자의 주관적 의견이 포함되어 참고용입니다.
